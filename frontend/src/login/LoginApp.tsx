...
export function LoginApp() {
  ...
  const submit = async (event: JSX.TargetedSubmitEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(
        t('Failed to sign in, please verify your account information. Devices with additional authentication enabled can continue under the default LuCI login.'),
      );
      window.requestAnimationFrame(() => passwordInput.current?.focus());
    } catch {
      setError(
        t('Unable to communicate with router. Please check your network connection and try again.'),
      );
    } finally {
      setPhase('ready');
    }
    ...
    <main class="ssh-login-page">
      <section class="ssh-login-brand" aria-labelledby="ssh-login-brand-title">
        ...
        <div class="ssh-login-brand-content">
          ...
          <p class="ssh-login-eyebrow">{t('SMART NETWORK PROTECTION')}</p>
          <h1 id="ssh-login-brand-title">{t('SmartSafeHub')}</h1>
          <p class="ssh-login-brand-description">
            {t('Make your family\'s internet safer and easier to manage. From protected status to Wi-Fi and connected devices, all in one place.')}
          </p>

          <div class="ssh-login-features" aria-label={t('SmartSafeHub Key Features')}>
            <LoginFeature
              description={t('Quickly manage DNS-based protection and local blocking rules.')}
              icon={<ShieldIcon />}
              title={t('SafeShield Protection')}
            />
            <LoginFeature
              description={t('Easily check the status of devices connected to your wireless network.')}
              icon={<WifiIcon />}
              title={t('Manage your network with ease')}
            />
            <LoginFeature
              description={t('Use the LuCI authentication and session policy of the current router as it is.')}
              icon={<KeyIcon />}
              title={t('On-device verification')}
            />
          </div>
        </div>
      </section>
        ...
        <div class="ssh-login-panel-inner">
          {phase === 'probing' ? (
            <div class="ssh-login-probe" role="status" aria-live="polite">
              ...
              <strong>{t('Checking SmartSafeHub connection')}</strong>
              <span>{t('You are checking an existing login session.')}</span>
            </div>
          ) : (
            <div class="ssh-login-card">
              ...
              <p class="ssh-login-kicker">{t('Manage SmartSafeHub')}</p>
              <h2 id="ssh-login-title">{t('Welcome back')}</h2>
              <p class="ssh-login-subtitle">{t('Please log in with your router management account.')}</p>
              ...
              <form class="ssh-login-form" onSubmit={submit} noValidate>
                <label class="ssh-login-field" for="smartsafehub-username">
                  <span class="ssh-login-label">{t('Your username')}</span>
                ...
                <label class="ssh-login-field" for="smartsafehub-password">
                  <span class="ssh-login-label">{t('Password')}</span>
                  <span class="ssh-login-input-wrap">
                    ...
                    <button
                      ...
                      type="button"
                    >
                      {showPassword ? t('HIDDEN') : t('Response Options')}
                    </button>
                  </span>
                </label>
