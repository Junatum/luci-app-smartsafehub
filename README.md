...
Pro/Ultimate 장치에서는 `/usr/libexec/smartsafehub-activity-sync`가 같은 이벤트의 Cloud outbox를 `/api/v1/activity/events`로 batch 전송합니다. `/api/v1/licenses/resolve`가 발급한 `activity_history` upload token/URL을 `/tmp`에만 보관하고, 전송 성공 응답의 `received == accepted + duplicates + expired`가 snapshot batch와 일치할 때 그 snapshot의 `event_id`만 ack합니다. 업로드 실패나 불완전 응답에서는 ack하지 않습니다. 유료 플랜인데 credential이 누락되거나 Hub Activity API가 아직 배포되지 않았거나 구 `/licenses/resolve` 응답을 반환하면 배포 지연/서버 오류로 보고 outbox를 보존해 재시도하며, legacy top-level `plan`도 인식합니다. Cloud-only outbox는 서버가 명시적으로 `Free`라고 확인한 경우에만 비우고, plan 미확인/불완전 응답에서는 보존합니다. Cloud ack는 로컬 `activity-history.jsonl`을 삭제하지 않습니다. 기본 주기 검사는 5분이며 새 이벤트가 생기면 wake marker로 빠르게 한 번 시도하되, 서버 통신 실패 시 marker를 계속 남겨 15초마다 재시도하지 않고 다음 정기 주기에 재시도합니다. 동기화 중 새 이벤트가 생기면 별도의 새 marker가 남아 다음 tick에서 처리됩니다. 공유기 `최근 활동` 화면에서는 entitlement, 전송 대기 건수, 마지막 성공 시각과 서버 retention을 함께 확인할 수 있습니다.
